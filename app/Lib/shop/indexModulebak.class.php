...
class indexModule extends ShopBaseModule
{
	...
	{		
		...
		{		
			...
			$GLOBALS['tmpl']->assign("notice_list", $notice_list);
			//输出公告
			$notice_list = get_notice(5, array(0, 1));
			$GLOBALS['tmpl']->assign("notice_list", $notice_list);
			//获取推荐的大分类
			$bcate_list = load_dynamic_cache("INDEX_RECOMMEND_BASECATE");
			if ($bcate_list === false) {
				$bcate_list = $GLOBALS['db']->getAll("select * from " . DB_PREFIX . "deal_cate where is_effect = 1 and is_delete = 0 and recommend = 1 order by sort desc");
				set_dynamic_cache("INDEX_RECOMMEND_BASECATE", $bcate_list);
				$GLOBALS['tmpl']->assign("bcate_list", $bcate_list);
			}
			
			//推荐分类的产品
			if ($bcate_list) {
				$datalist = load_dynamic_cache("INDEX_RECOMMEND_CATLIST");
				if ($datalist === false) {
					foreach ($bcate_list as $value) {
						$iCatId = $value['id'];
						//获取下集分类
						...
						$sort_field = "sort";
						$sort_type = "desc";
						$result = get_deal_list(6, $iCatId, 0, $type = array(DEAL_ONLINE), $where, $sort_field . " " . $sort_type);
						$prolist = $result['list'];
						$value['plist'] = $prolist;
						$datalist[] = $value;
					}
					set_dynamic_cache("INDEX_RECOMMEND_CATLIST", $datalist);
				}
	...
	public function index_left_store()
	{
		...
		$store_list2 = $GLOBALS['db']->getAll("SELECT * FROM ".DB_PREFIX."supplier_location  use index(avg_point) WHERE  city_id in(".implode(",",$city_ids).") and is_effect = 1 order by avg_point desc limit 10");
		}
		else
		{
		...
		$store_list2 = $GLOBALS['db']->getAll("SELECT * FROM ".DB_PREFIX."supplier_location  use index(avg_point) WHERE  is_effect = 1 order by avg_point desc limit 10");
		}
		
		$bcate_list = load_dynamic_cache("INDEX_RECOMMEND_BCATE");
		$GLOBALS['tmpl']->assign("bcate_list",$bcate_list);
		$GLOBALS['tmpl']->assign("store_list",$store_list);
		$GLOBALS['tmpl']->assign("store_list2",$store_list2);
		return $GLOBALS['tmpl']->fetch("index/index_left_store.html");
	}
	public function index_left_tuan()
	{
		$tuan_list = get_deal_list_show(0,0,$GLOBALS['deal_city']['id'],array(DEAL_ONLINE),"");
		$tuan_list =$tuan_list['list'];
		$tuan_list2 = get_deal_list_show(10,0,$GLOBALS['deal_city']['id'],array(DEAL_ONLINE),""," buy_count desc,sort desc ");
		$tuan_list2 =$tuan_list2['list'];
		$GLOBALS['tmpl']->assign("tuan_list",$tuan_list);
		$GLOBALS['tmpl']->assign("tuan_list2",$tuan_list2);
		$bcate_list = load_dynamic_cache("INDEX_RECOMMEND_BCATE");
		$GLOBALS['tmpl']->assign("bcate_list",$bcate_list);
	...
	public function index_left_youhui()
	{
		...
		$youhui_list2 = get_free_youhui_list(10,0," "," view_count desc ");
		$youhui_list2 = $youhui_list2['list'];
		$GLOBALS['tmpl']->assign("youhui_list",$youhui_list);	
		$GLOBALS['tmpl']->assign("youhui_list2",$youhui_list2);	
		$bcate_list = load_dynamic_cache("INDEX_RECOMMEND_BCATE");
		$GLOBALS['tmpl']->assign("bcate_list",$bcate_list);
	...
	public function index_left_daijin()
	{
		...
		$result2 = search_youhui_list(10,0,"","",false,"",$GLOBALS['deal_city']['id']);
		$daijin_list2 = $result2['list'];
		$GLOBALS['tmpl']->assign("daijin_list2",$daijin_list2);
	...
	public function index_left_event()
	{
		...
		$event_list2  = search_event_list(5,0,$GLOBALS['deal_city']['id'],""," submit_count desc ");
		$event_list2 = $event_list2['list'];
		$GLOBALS['tmpl']->assign("event_list2",$event_list2);
