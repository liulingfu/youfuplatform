<?php
//代金券的切换菜单
class byouhui_filter_nav_cache_auto_cache extends auto_cache{
	public function load($param)
	{
		...
		if($result===false)
		{
			...
			foreach($area_list as $k=>$v)
			{
					...
					$area_list[$k]['url'] = url("youhui","ycate",$tmp_url_param);	
					
					$condition = build_deal_filter_condition($tmp_url_param);
					$area_list[$k]['count'] = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal where is_effect = 1 and is_delete = 0 and is_shop = 2 and (end_time = 0 or end_time > '".get_gmtime()."') $condition ");
			}		
				...
				foreach($quan_list as $k=>$v)
					{
						...
						$quan_list[$k]['url'] = url("youhui","ycate",$tmp_url_param);	
						$condition = build_deal_filter_condition($tmp_url_param);
					$quan_list[$k]['count'] = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal where is_effect = 1 and is_delete = 0 and is_shop = 2 and (end_time = 0 or end_time > '".get_gmtime()."') $condition ");
				}			
			}
			...
			foreach($cate_list as $k=>$v)
			{
					...
					$cate_list[$k]['url'] = url("youhui","ycate",$tmp_url_param);	
						$condition = build_deal_filter_condition($tmp_url_param);
					$cate_list[$k]['count'] = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal where is_effect = 1 and is_delete = 0 and is_shop = 2 and (end_time = 0 or end_time > '".get_gmtime()."') $condition ");
			...
			{			
				...
				{									
							...
							$durl = url("youhui","ycate",$tmp_url_param);						
							$scate_list[$k]['url'] = $durl;
								$condition = build_deal_filter_condition($tmp_url_param);
					$scate_list[$k]['count'] = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal where is_effect = 1 and is_delete = 0 and is_shop = 2 and (end_time = 0 or end_time > '".get_gmtime()."') $condition ");
